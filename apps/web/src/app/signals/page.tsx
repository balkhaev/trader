    ...
    <div className="rounded-xl border bg-card/70 p-4 transition-colors hover:border-primary/30 hover:bg-card">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div className="rounded-lg border bg-background/40 p-2.5">
          ...
          <div className="mt-1 font-mono text-xs">
            {payload?.entryPrice
              ? Number(payload.entryPrice).toPrecision(6)
              : "next bar"}
          </div>
        </div>
        <div className="rounded-lg border bg-background/40 p-2.5">
          ...
          <div className="mt-1 font-mono text-xs">
            {payload?.stopPrice
              ? Number(payload.stopPrice).toPrecision(6)
              : "—"}
          </div>
        </div>
        <div className="rounded-lg border bg-background/40 p-2.5">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-border/60 border-t pt-3 text-[10px] text-muted-foreground">
        <div className="flex flex-wrap gap-x-4 gap-y-1 font-mono">
          ...
          {payload?.maxHoldMinutes ? (
            <span>MAX HOLD {payload.maxHoldMinutes}m</span>
          ) : null}
        </div>
...
function EmptyState({ title, text }: { title: string; text: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="mt-1 max-w-md text-muted-foreground text-xs leading-5">
        {text}
      </p>
    </div>
  );
...
export default function StrategySignalsPage() {
  ...
  const candidates = strategySignals.filter(
    (signal) => signal.status === "pending"
  );
  const executed = strategySignals.filter(
  ...
  const winRate = strategyClosed.length
    ? (wins.length / strategyClosed.length) * 100
    : 0;
  ...
  return (
    ...
    >
      <StatRow className="md:grid-cols-5">
        ...
        <StatItem
          label="Win rate"
          value={strategyClosed.length ? `${winRate.toFixed(1)}%` : "—"}
        />
        <StatItem
          label="Closed return"
          value={
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-muted-foreground text-xs leading-5">
              Универсальных AI-сигналов на этом экране нет. Pending создаётся
              scanner, а исполнение разрешается только при активной стратегии,
              Binance account и execution preflight.
            </p>
          </div>
        </div>
      </div>
      ...
      <Tabs className="mt-4" onValueChange={setTab} value={tab}>
        ...
        <TabsContent className="mt-4" value="closed">
          <TerminalPanel
            subtitle="Realized strategy trades"
            title="Closed Trades"
          >
            {strategyClosed.length ? (
              <div className="grid gap-3 p-3 xl:grid-cols-2">
                {strategyClosed.map((signal) => (
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center justify-between border-border/60 border-t pt-3 text-[10px] text-muted-foreground">
                      <span>HOLD {signal.holdingPeriodMinutes ?? "—"}m</span>
                      <span>
                        {signal.exitAt
                          ? new Date(signal.exitAt).toLocaleString("ru-RU")
                          : "—"}
                      </span>
                    </div>
                  </div>
        ...
        <TabsContent className="mt-4" value="rejected">
          <TerminalPanel
            subtitle="Skipped, rejected and expired"
            title="Rejected"
          >
            {rejected.length ? (
              <div className="grid gap-3 p-3 xl:grid-cols-2">
      ...
      <div className="mt-4 flex items-start gap-3 rounded-xl border border-yellow-500/30 bg-yellow-500/5 p-4">
        ...
        <div>
          <p className="font-medium text-sm">
            Performance считается только по закрытым strategy trades
          </p>
          <p className="mt-1 text-muted-foreground text-xs leading-5">
            Общие news/LLM/legacy signals исключены из локальных показателей
            этого экрана. Forward gate требует минимум 30 новых сделок и
            положительный результат без трёх лучших.
          </p>
        </div>
      </div>
    </PageLayout>
